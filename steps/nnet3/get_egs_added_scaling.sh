...
#Added by me (USed when needed to scale a particular accent's posterior)
scale=false
scale_file=""
scale_factor=-1
#Added by me
...
if [ $# != 3 ]; then
  ...
  echo "                                                   # the middle."
  echo "  --scale <scale|true>                             # Used to scale a particular accents examples' posteriors. Need to provide"
  echo "                                                   # --scale-factor and --scale-file if true."
  echo "  --scale-file                                     # Specify the file containing the utterances whose posteriors neede to be "
  echo "                                                   # scaled. Needed when --scale is true"
  echo "  --scale-factor                                   # Used to specify the factor by which to scale the posteriors of the utterances"
  echo "                                                   # given by --scale-file if --scale is true."
...
if [ $stage -le 3 ]; then
  ...
  $cmd $dir/log/create_valid_subset.log \
    ...
    ali-to-post ark:- ark:- \| \
    /home/abhinav/kaldi/accents/nnet3-get-egs-mod --num-pdfs=$num_pdfs --scale=$scale --scale-file=$scale_file --scale-factor=$scale_factor --frame-subsampling-factor=$frame_subsampling_factor \
