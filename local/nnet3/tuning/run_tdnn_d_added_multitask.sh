...
if [ $stage -le 9 ]; then
  ...
  relu-renorm-layer name=prefinal-affine input=tdnn6 dim=1024
  output-layer name=output dim=$num_targets max-change=1.5 presoftmax-scale-file=$dir/configs/presoftmax_prior_scale.vec

  relu-renorm-layer name=prefinal-affine-accent input=tdnn6 dim=1024
  output-layer name=output-accent input=tdnn6 dim=$num_accents max-change=1.5
EOF
...
if [ $stage -le 10 ]; then
  ...
  steps/nnet3/train_dnn_added_multitask.py --stage=$train_stage \
    --cmd="$decode_cmd" \
    ...
    --num-accents $num_accents \
    --accents-file $accents_file \
